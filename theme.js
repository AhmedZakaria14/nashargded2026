...
(function(){
  ...
  function syncButtons(){
    ...
    if(meta) meta.setAttribute('content',theme==='light'?'#f8fafc':'#07100c');
  }
  function setTheme(theme,save){
    root.dataset.theme=theme==='dark'?'dark':'light';
    if(save!==false){try{localStorage.setItem(KEY,root.dataset.theme)}catch(e){}}
    syncButtons();
    window.dispatchEvent(new CustomEvent('elnashar-theme-change',{detail:{theme:root.dataset.theme}}));
  }
  function makeButton(){
  ...
  function addOriginalDesktop(){
    ...
    if(!menu||menu.querySelector('.theme-toggle-item')) return;
    const li=document.createElement('li');li.className='theme-toggle-item menu-item';li.appendChild(makeButton());menu.appendChild(li);
  }
  ...
  function addSpaDesktop(){
    ...
    if(!nav||nav.querySelector('.theme-toggle-item')) return;
    const wrap=document.createElement('span');wrap.className='theme-toggle-item';wrap.appendChild(makeButton());nav.appendChild(wrap);
  }
  function addSpaMobile(){
    const nav=document.querySelector('.mobile-menu nav');
    if(!nav||nav.querySelector('.theme-toggle-item')) return;
    const wrap=document.createElement('div');wrap.className='theme-toggle-item';wrap.appendChild(makeButton());nav.appendChild(wrap);
  }
  function mount(){
