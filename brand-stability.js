...
(function(){
  ...
  function patchAttrs(el){
    ...
    if(el.tagName==='A'){
      ...
      if(/^mailto:[^?]*@(adselams\.com|nashar\.digital)/i.test(href)){
        ...
        if(el.childElementCount===0)el.textContent='تواصل مع '+BRAND;
  ...
  function patchHead(){
    ...
    const desc=textBrand(document.querySelector('meta[name="description"]')?.getAttribute('content')||'النشار جروب شريك نمو رقمي متكامل يقدم خدمات التسويق الرقمي والإعلانات وتحسين محركات البحث وتصميم المواقع في السعودية والخليج.');
  ...
  function start(){
    patchHead();
    if(document.body){patchSubtree(document.body);observe(document.body)}
    else{
      observe(document.documentElement);
      document.addEventListener('DOMContentLoaded',()=>{patchSubtree(document.body);patchHead()},{once:true});
    }
